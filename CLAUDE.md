...
1. ✅ **`engine/rules/`** — all ICT detection modules (Candle → ATR → Displacement → Swing → FVG → BOS/TrendState → CHOCH → Sweep → StructureState → event bus → replay/validation harness), plus Addendum A's Kill Zone filter and HTF Bias Cascade (`engine/rules/bias_cascade.py`). Built and tested (106 automated tests, `pytest`). No interface yet.
...
7. **`interface/dashboard.py`** *(next)* — Streamlit, built last, once the underlying signal is proven.
