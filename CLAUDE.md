...
5. ✅ **`main.py`** — the always-on orchestrator: one `StructureStateEngine` per timeframe needed for the HTF Bias Cascade (D/H4/H1/M15) plus the entry timeframe (M5), live candles in, alert-worthy events to Telegram, eligible setups to the AI evaluator + context layer, verdicts logged, Telegraphed, and desktop-notified. Live-verified against the owner's real MT5 account (connects, warms up all 5 timeframes, polls without error) - a full live signal→AI→Telegram firing hasn't been observed yet simply because none has occurred during a test window; the decision logic itself has dedicated unit tests. **The owner left this running live overnight on 2026-07-28 and it worked correctly** - stayed up for hours, found a real FVG and a real CHOCH, correctly withheld AI verdicts on setups that didn't clear the Kill Zone + Bias Cascade gate (this is expected behavior, not a bug).
...
9. ✅ **`engine/desktop_notifier.py`** — native Windows toast notifications for AI verdicts, owner's choice as a second alert channel independent of Telegram (which is still network-blocked). Uses `win11toast`'s `notify()` specifically, not `toast()` - confirmed `toast()` blocks until the user dismisses the popup, which would have stalled `main.py`'s live polling loop every time a verdict fired; `notify()` is fire-and-forget (~30ms). Wired into `_handle_setup()` plus start/stop/crash notifications. Live-verified: real toast confirmed visible on the owner's machine.
10. **All of Phase 1 + Build Steps 1-5, plus the backup utility and desktop notifications, are done.** Remaining work is optional/future: Phase 2 ICT concepts (Order Blocks, Breaker Blocks, OTE, Judas Swing, Power of Three - Section 4.12), expanding instruments, or a database if scale genuinely requires it.
...
**Gap found and fixed on 2026-07-28, while checking on the overnight live run:** the test suite and any live `main.py` session shared the same `logs/` directory, so running `pytest` interleaved test noise directly into production logs (a simulated "MT5 hiccup" from `tests/test_mt5_bridge.py` briefly looked like a real connection error). `get_logger()` now reads its directory from `ICT_LOGS_DIR`, which `tests/conftest.py` points at a throwaway temp folder - confirmed by deleting `logs/` and running the full suite, which no longer recreates it.
...
**Operational note:** if `main.py` is left running across a session where code changes get merged (as happened on 2026-07-28 - the owner had it running live while several fixes landed), it keeps using whatever code was loaded in memory at startup. It needs a restart (Ctrl+C, then `python main.py` again) to actually pick up new fixes - merging files to disk doesn't hot-reload a running Python process.

**Housekeeping item found, not yet fixed (2026-07-28):** unlike `.env`, `venv/` is tracked in git but was never added to `.gitignore` - every `pip install` this session has been silently drifting hundreds of tracked venv files (pip's own internals get replaced on almost any install). Nothing has been committed from this drift (every commit this session explicitly excluded `venv/`), so no harm done, but it's worth properly untracking `venv/` from git the same way `.env` was fixed, so `git status` stops showing venv noise on every check. Owner hasn't been asked about this yet - flag it and confirm before running `git rm -r --cached venv/`, since unlike `.env` (which was empty in every commit) this would be removing hundreds of real, previously-committed files from tracking.
